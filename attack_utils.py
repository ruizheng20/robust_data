...
from textattack.datasets.dataset import Dataset
...
def dataset_script_path(name):
    if name=="glue":
        return "/root/dataset_scripts/glue"
    else:
        return name
...
class HuggingFaceDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(name_or_dataset, datasets.Dataset):
            ...
            self._subset = subset
            if self._name == "SetFit/20_newsgroups":
                self._dataset = datasets.load_dataset(dataset_script_path(self._name), subset)["train"]
                total_size = len(self._dataset)
                train_size = int(total_size * 0.85)
                test_size = total_size - train_size
                self._dataset = self._dataset.train_test_split(total_size - train_size, train_size, seed=42)
                if split == "train":
                    self._dataset = self._dataset["train"]
                else:
                    self._dataset = self._dataset["test"]
            else:
                self._dataset = datasets.load_dataset(dataset_script_path(self._name), subset)
